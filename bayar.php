<?php 
require 'functions.php';

if (isset($_POST['cariNis'])) {
    $dataSiswa = cariData($_POST['nis']);
    // Query tb_Bulan
    $dataBulan = query("SELECT * FROM tb_bulan");
}

if(isset($_POST['bayarSPP'])) {

    if( bayarSpp($_POST) > 0 ) {
        echo "
        <script>
        alert('Berhasil membayar spp siswa!');
        document.location.href = 'index.php'
        </script>
        ";
    } else {
        echo "
        <script>
        alert('Gagal membayar spp siswa!');
        </script>
        ";

        var_dump(mysqli_affected_rows($conn));
    }
    
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bayar SPP</title>
</head>
<style>
    body {
      padding: 0;
      margin: 0;
    }
    nav {
      width: 100%;
      height: 50px;
      background-color: #9999;
      display: flex;
      justify-content: center;
      gap: 50px;
      align-items: center;
    }
    nav a {
      text-decoration: underline;
      color: black;
      font-size: 20px;
    }
  </style>
<body>
<nav>
  <a href="index.php">Daftar Siswa</a>
  <a href="histori.php">Histori Pembayaran Siswa</a> <br>
  <a href="bayar.php">Bayar SPP</a>
  <a href="tambahsiswa.php">Tambah data siswa</a>
</nav>
<H2>Cari nis siswa yang akan membayar spp</H2>
    <form method="post">
        <label for='nis'>NIS :</label>
        <input type='text' id='nis' name="nis">

        <button type="submit" name="cariNis">Cari</button>
    </form>
    <a href="index.php">Kembali</a>
<?php if( isset($_POST['cariNis'])) : ?>
    <form method="post">
        <ul>
            <li>
            <label for='nis'>NIS Siswa</label>
            <input type='text' id='nis' name="nis" value="<?= $dataSiswa[0]['nis']?>">
            </li>
            <li>
            <label for='nama'>Nama Siswa</label>
            <input type='text' id='nama' name="nama" value="<?= $dataSiswa[0]['nama']?>">
            </li>
            <li>
            <label for='kelas'>Kelas</label>
            <input type='text' id='kelas' name="kelas" value="<?= $dataSiswa[0]['kelas']?>">
            </li>
            <li>
            <label for='tgl'>Tanggal Bayar</label>
            <input type='date' id='tgl' name="tgl" required>
            </li>
            <li>
            <label for='bulan'>Bayar Bulan</label>
            <select name="bulan" id="bulan">
                <?php foreach( $dataBulan as $bulan  ) : ?>
                    <option value="<?= $bulan['bulan'] ?>" required><?= $bulan['bulan'] ?></option>
                    <?php endforeach; ?>
            </select>
            </li>
            <li>
            <label for='bayar'>Nominal Bayar</label>
            <input type='text' id='bayar' name="bayar" required>
            </li>
            <li>
            <label for='keterangan'>Keterangan</label>
            <input type='text' id='keterangan' name="keterangan" required>
            </li>
        </ul>
        <button type="submit" name="bayarSPP">Bayar Spp</button>
    </form>
<?php endif; ?>
</body>
</html>